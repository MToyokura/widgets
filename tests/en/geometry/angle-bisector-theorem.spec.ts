...
const PATH = "/en/geometry/angle-bisector-theorem/";

test.describe("Angle Bisector Theorem (en)", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(`${BASE}${PATH}`);
  });

  test("should have the correct title", async ({ page }) => {
    await expect(page).toHaveTitle(/Angle Bisector Theorem/);
    await expect(
  ...
  test("should have a slider to control steps", async ({ page }) => {
    // The label is hardcoded as "Step" in the component
